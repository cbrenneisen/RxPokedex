...
//            updateDates ? WildPokemonSection(number: $0.number, pokemon: $0.pokemon.map { x in Pokemon(number: x.number, name: x.name, image: x.image, date: date) },
...
//
//        // insert sections
//        for (i, section) in self.unusedSections.enumerated() {
//            (nextRng, randomValue) = nextRng.get_random()
...
//
//                sections.insert(WildPokemonSection(number: i, pokemon: [], updated: Date.distantPast ), at: index)
//            }
//            else {
...
//                }
//
//                nextUnusedSections.append(String(section.number))
//            }
//        }
//
//        assert(countTotalItems(sections: sections) + nextUnusedItems.count == startItemCount)
